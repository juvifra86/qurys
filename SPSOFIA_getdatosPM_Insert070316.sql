...
SPSOFIA_getdatosPM_Insert070316 2,'20160229',5                                
...
CREATE procedure SPSOFIA_getdatosPM_Insert070316                                
@EmpresaID integer,                                    
@Periodo Date, ---Fecha              
...
--     --,dbo.FNSOFIA_GetCadenaBC('N', cast( c.saldo as int)  ,20)[ImportePago]                  
...
--     --,dbo.FNSOFIA_GetCadenaBC('N','',8)[DteLiquida]                                    
...
--     --, dbo.FNSOFIA_GetCadenaBC('N', 0,3)[VencidoDE1]                                
                                
--     , case when isnull(d.SaldoTotalBoleta,0) + isnull(d.SaldoTotalCapital,0) < = 0 then dbo.FNSOFIA_GetCadenaBC('N',0,20)                                
--       else dbo.FNSOFIA_GetCadenaBC('N', Case BoletasXVencer when 0 then 0 ---si tiene boletas por vencer se pone como vigente si no se manda cero                    
--   --else  (cast(d.SaldoCapital as Int) + cast(d.SaldoInteres as Int)+ cast(d.SaldoTotalCapital as Int)) End , 20)                                
--   else  (cast(d.SaldoTotalCapital as Int)) End , 20)  ---Solo dejamos la deuda de puro capital futuro                               
--   end  ---CANTIDAD                                
                                       
--     --, dbo.FNSOFIA_GetCadenaBC('N', Case BoletasXVencer when 0 then 0 else  (cast(d.SaldoCapital as Int) + cast(d.SaldoInteres as Int)+ cast(d.SaldoTotalCapital as Int)) End , 20)[Cantidad]                                
--     , space(75)[FillDE1]                                                              
...
--     , --case when isnull(d.SaldoTotalBoleta,0) + isnull(d.SaldoTotalCapital,0) < = 0 then dbo.FNSOFIA_GetCadenaBC('N',0,3)                                
--      --else dbo.FNSOFIA_GetCadenaBC('N', case when d.diasatraso > 999 then 999 else d.diasatraso end,3) end ----DIAS DE VENCIMIENTO  [VencidoDE2]                                   
--      dbo.FNSOFIA_GetCadenaBC('N', case when d.diasatraso > 999 then 999 else d.diasatraso end,3)                  
                        
--     --, dbo.FNSOFIA_GetCadenaBC('N', case when d.diasatraso > 999 then 999 else d.diasatraso end,3)[VencidoDE2]                                   
                      
--  ,case when isnull(d.SaldoTotalBoleta,0) + isnull(d.SaldoTotalCapital,0) < = 0 then dbo.FNSOFIA_GetCadenaBC('N',0,20)                                
--  --else (Case cast(d.SaldoTotalBoleta as Int) when 0 then 0 else  (cast( d.SaldoCapital as Int) + cast( d.SaldoInteres as Int)+ cast( d.SaldoTotalCapital as Int)) End)  --[CantidadDE2]                                    
--  else (Case cast(d.SaldoTotalBoleta as Int) when 0 then 0 else cast( d.SaldoTotalBoleta as Int) End)  --[CantidadDE2]   ------solo manda lo vencido                                 
--  end --CANTIDAD                                
                                        
--     --,(Case cast(d.SaldoTotalBoleta as Int) when 0 then 0 else  (cast( d.SaldoCapital as Int) + cast( d.SaldoInteres as Int)+ cast( d.SaldoTotalCapital as Int)) End)  --[CantidadDE2]                                    
--     , space(75)[FillDE2]                                                            
                                     
--     ---------------------------------------------------------SumaSaldoVencido----------------------------------------------------------------------------------------------------------------------------                        
--    ,--(Case BoletasXVencer when 0 then 0 else                    
--   cast( d.SaldoTotalBoleta as Int)  ---------Solo se manda lo vencido                  
--  --(cast( d.SaldoCapital as Int) + cast( d.SaldoInteres as Int)+ cast( d.SaldoTotalCapital as Int)) End)  --[CantidadDE1]                                                        
--        --+ (Case cast(d.SaldoTotalBoleta as Int) when 0 then 0 else  (cast( d.SaldoCapital as Int) + cast( d.SaldoInteres as Int)+ cast( d.SaldoTotalCapital as Int))                   
--   -- End)  --[CantidadDE2]                                                        
...
Inner Join CarteraCte d          On c.CuentaCtekey = d.CuentaCteId                                          
                                 And cast(d.FechaCorte as date)=cast (@UltimaFecha as date)              
